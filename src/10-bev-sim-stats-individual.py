...
class EVSimStats:
    ...
    def stats(self, paraset=None):
        ...
        vars_sql = ','.join(vars)
        start = time.time()
        df = pd.read_sql(f'''SELECT {vars_sql} FROM scenario_vg_car_ev_sim_paraset{paraset}_5days;''',
                         con=engine)
        ...
        tqdm.pandas()
        df_stats = df.groupby('person').progress_apply(lambda data: ev.ev_sim_multi_stats(data, paraset=paraset,
                                                                                          df_para=self.df_para)).reset_index()
        df_stats.to_sql(name=self.scenario + '_individual_5days',
...
if __name__ == '__main__':
    ...
    for paraset in [0, 1, 2, 3]:
        print(f'Processing parameter set {paraset} for simulation...')
